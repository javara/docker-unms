...
if [[ "$(dpkg -l apt | grep ^"ii" | awk '{print $2,$3}' | awk '{print $2}' | cut -d'.' -f1)" -ge "1" ]] && [[ "$(dpkg -l apt | grep ^"ii" | awk '{print $2,$3}' | awk '{print $2}' | cut -d'.' -f2)" -ge "1" ]]; then if ! grep -q "allow-change-held-packages" /tmp/EUS/apt_option &> /dev/null; then echo ""${apt_options[@]}"" &>> /tmp/EUS/apt_option; fi; fi
...
# Check if MongoDB is newer than 2.6 for UniFi Network application 7.4.x
if [[ "${first_digit_unifi}" -gt '7' ]] || [[ "${first_digit_unifi}" == '7' && "${second_digit_unifi}" -ge '4' ]]; then
  mongodb_server_version=$(dpkg -l | grep "^ii\\|^hi" | grep "mongodb-server \\|mongodb-org-server " | awk '{print $3}' | sed 's/\.//g' | sed 's/.*://' | sed 's/-.*//g')
  if [[ "${mongodb_server_version::2}" -lt "25" ]]; then
    header_red
    unifi_latest_73=$(curl -s "https://get.glennr.nl/unifi/latest-versions/7.3/latest.version")
    echo -e "${WHITE_R}#${RESET} UniFi Network Application ${first_digit_unifi}.${first_digit_unifi}.${second_digit_unifi} requires MongoDB 2.6 or newer."
    echo -e "${WHITE_R}#${RESET} The latest version that you can run with MongoDB version $(dpkg -l | grep "^ii\\|^hi" | grep "mongodb-server \\|mongodb-org-server " | awk '{print $3}') is ${unifi_latest_73} and older.. \\n\\n"
    echo -e "${WHITE_R}#${RESET} Consider upgrading MongoDB to version 2.6 or newer, or perform a fresh install using my scripts (on the latest OS):"
